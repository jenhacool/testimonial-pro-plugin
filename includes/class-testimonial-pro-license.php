 ...
 */
if ( ! class_exists( 'Testimonial_Pro_License' ) ) {
	/**
	 * Testimonial_Pro_License class to handle license.
	 *
	 * @since 2.2.4
	 */
	class Testimonial_Pro_License {
		...
		private function includes() {
			if ( ! class_exists( 'Testimonial_Pro_Auto_Update' ) ) {
				require_once __DIR__ . '/class-testimonial-pro-auto-update.php';
			}
		}
		...
		public function init_updater() {
			...
			// Setup the updater.
			$edd_updater = new Testimonial_Pro_Auto_Update(
				$this->api_url,
				$this->file,
				$args
			);
...
$base_url = admin_url( 'edit.php?post_type=spt_testimonial&page=tpro_settings#tab=1' );
update_option( $this->get_license_status_option_key(), $license_data );
...
exit();
		...
		public function license_active_notices() {
			...
			if ( 'valid' == $license_status ) {
				...
				$this->show_error(
					sprintf(
						__( 'Your <strong>Testimonial Pro</strong> license key has been expired. Please <a href="%1$s" target="_blank">renew license key</a> at discount.', 'testimonial-pro' ),
				...
				echo sprintf(
					__( '<div class="testimonial-pro-license-notice notice"><div class="testimonial-pro-notice-icon"><p>You must activate the license key to make the <strong><a href="%1$s" target="_blank">Testimonial Pro</a></strong> plugin work, get automatic updates & support.</p><a href="%2$s" class="testimonial-pro-activate-btn">Activate</a></div></div>', 'testimonial-pro' ),
		...
		public function plugin_row_license_missing( $plugin_data, $version_info ) {
			...
			if ( ( ! is_object( $license ) || 'valid' !== $license->license ) && empty( $showed_missing_key_message[ $this->get_license_option_key() ] ) ) {
				...
				echo '&nbsp;<strong><a href="' . esc_url( admin_url( 'edit.php?post_type=spt_testimonial&page=tpro_settings#tab=1' ) ) . '">' . esc_html__( 'Enter valid license key for automatic updates.', 'testimonial-pro' ) . '</a></strong>';
