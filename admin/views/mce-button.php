...
if ( ! class_exists( 'SP_TPRO_MCE_Shortcode_list' ) ) {
	class SP_TPRO_MCE_Shortcode_list {
		...
		public function __construct() {
			add_action( 'wp_ajax_tpro_cpt_list', array( $this, 'tpro_list_ajax' ) );
			add_action( 'admin_footer', array( $this, 'tpro_cpt_list' ) );
			add_action( 'admin_head', array( $this, 'tpro_mce_button' ) );
		}

		// Hooks your functions into the correct filters.
		function tpro_mce_button() {
		...
		function add_mce_plugin( $plugin_array ) {
			$plugin_array['sp_tpro_mce_button'] = SP_TPRO_URL . 'admin/assets/js/mce-button.js';
			return $plugin_array;
		}

		// Register our button in the editor.
		function register_mce_button( $buttons ) {
			array_push( $buttons, 'sp_tpro_mce_button' );
			return $buttons;
		}
		 ...
		 * @return string
		 */
		public function tpro_list_ajax() {
			// check for nonce.
			...
			if ( $current_screen == 'post' || 'page' ) {
				...
				<script type="text/javascript">
					jQuery( document ).ready( function( $ ) {
						var data = {
							'action'	: 'tpro_cpt_list',	// wp ajax action.
						...
						jQuery.post( ajaxurl, data, function( response ) {
							...
							if( response === '-1' ){
								...
								if (typeof(tinyMCE) != 'undefined') {
									...
									tinyMCE.activeEditor.settings.spTPROShortcodeList = response;
								}
							}
							}
						});
 ...
 *  Kicking this off
 */
$sp_mce_btn = new SP_TPRO_MCE_Shortcode_list();
$sp_mce_btn->init();
