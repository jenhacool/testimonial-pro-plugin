...
function sp_testimonial_pro_widget() {
	register_widget( 'TPRO_Widget_Content' );
}
...
class TPRO_Widget_Content extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'TPRO_Widget_Content', __( 'Testimonial Pro', 'testimonial-pro' ),
			array(
	...
	public function form( $instance ) {
		...
		if ( count( $shortcodes ) > 0 ) {
			...
			echo sprintf( '<p><label for="%1$s">%2$s</label>', $this->get_field_id( 'title' ), __( 'Title:', 'testimonial-pro' ) );
			...
			echo sprintf( '<p><label>%s</label>', __( 'Testimonial Shortcodes:', 'testimonial-pro' ) );
			...
			echo sprintf(
				...
				__( 'You did not generate any shortcode yet.', 'testimonial-pro' ),
				__( 'to generate a new shortcode now.', 'testimonial-pro' ),
				__( 'click here', 'testimonial-pro' )
			);
		}
	}
