	...
	class SPFTESTIMONIAL_Field_image_sizes extends SPFTESTIMONIAL_Fields {
		...
		public function render() {
			...
			$sizes = array_merge(
				$sizes,
				array(
					'full'   => __( 'Original uploaded image', 'testimonial-pro' ),
					'custom' => __( 'Set custom size', 'testimonial-pro' ),
				)
			);
			...
			if ( ! empty( $sizes ) ) {
				...
				if ( ! empty( $sizes ) ) {
					...
					echo ( ! empty( $this->field['empty_message'] ) ) ? $this->field['empty_message'] : esc_html__( 'No image sizes found.', 'testimonial-pro' );
