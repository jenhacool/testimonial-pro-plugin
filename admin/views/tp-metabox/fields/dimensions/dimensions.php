  ...
  class SPFTESTIMONIAL_Field_dimensions extends SPFTESTIMONIAL_Fields {
    ...
    public function render() {
      ...
      $args = wp_parse_args( $this->field, array(
        ...
        'width_placeholder'  => esc_html__( 'width', 'testimonial-pro' ),
        'height_placeholder' => esc_html__( 'height', 'testimonial-pro' ),
        'width'              => true,
        'height'             => true,
        'unit'               => true,
        'show_units'         => true,
      ...
      if( ! empty( $args['width'] ) ) {
        ...
        echo ( ! empty( $args['width_icon'] ) ) ? '<span class="spftestimonial--label spftestimonial--label-icon">'. $args['width_icon'] .'</span>' : '';
        echo '<input type="number" name="'. $this->field_name('[width]') .'" value="'. $value['width'] .'"'. $placeholder .' class="spftestimonial-number" />';
      ...
      if( ! empty( $args['height'] ) ) {
        ...
        echo '<div class="spftestimonial--input">';
        echo ( ! empty( $args['height_icon'] ) ) ? '<span class="spftestimonial--label spftestimonial--label-icon">'. $args['height_icon'] .'</span>' : '';
        echo '<input type="number" name="'. $this->field_name('[height]') .'" value="'. $value['height'] .'"'. $placeholder .' class="spftestimonial-number" />';
        echo ( count( $args['units'] ) === 1 && ! empty( $args['unit'] ) ) ? '<span class="spftestimonial--label spftestimonial--label-unit">'. $args['units'][0] .'</span>' : '';
        echo '</div>';
